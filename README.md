...
- **`pipeline/dialog.py`** (Person B) — `ConversationBrain`, a clarification-attribute priority policy. Now wired into `agent.py` as an opt-in `ask_policy` (`--ask-policy dialog_fixed` / `dialog_simulator`), used only to choose *which attribute to ask* — it does not feed retrieval, which still reads its own `state.constraints`. **Not the default**, because it measurably underperforms on every condition harder than the public set; see below.
...
- **B's dialog policy is wired in but not the default, and the reason is a real, measured finding worth flagging.** `pipeline/dialog.py`'s `ConversationBrain` is now used via `--ask-policy {dialog_fixed,dialog_simulator}`, but the shipped default remains the `"other"` wildcard placeholder, because on every condition tested it wins:

  | ask policy | public (200) | paraphrase L3 | held-out (200) |
  |---|---|---|---|
  | `other` (default) | **0.9538** | **0.6675** | **0.8941** |
  | `dialog_simulator` | 0.9339 | 0.5528 | 0.8502 |
  | `dialog_fixed` | 0.9136 | 0.5267 | 0.8236 |

  (An earlier version of this table reported a held-out score of 0.6035 for `dialog_simulator` — that number was itself a measurement bug: a leftover paraphrase-harness monkeypatch from a prior robustness sweep had not been reset, so it was actually held-out-under-heavy-paraphrase, not clean held-out. Caught by re-verifying with an explicit reset before each measurement, the same discipline applied throughout this repository. The real gap is real but much smaller than that number implied.)

  The margin is modest on the public set (−0.02) and on held-out (−0.04), and largest specifically under paraphrase (−0.11 to −0.14 at L3). The likely mechanism: the simulator only discloses a constraint when the asked attribute's type matches an undisclosed one, so `"other"`'s blind wildcard is immune to parsing accuracy, while `ConversationBrain`'s own text extractors (`_extract_category`, `_extract_revealed_constraints`, etc.) use exact `startswith()` prefix matching with no fallback for reworded phrasing — unlike `pipeline/router.py`, which falls back to raw-text search when its own parsing fails. Under heavy paraphrase, that brittleness costs more disclosed information per turn, which compounds with the exposure gate's reliance on early-turn information density.

  `"other"` is a legitimate value under the API contract, but it is also the same category of concern as the exposure gate before its rework: it produces a repetitive transcript ("Anything else that matters to you?" every turn) that a judge reading the demo would likely find less convincing than named, specific questions, in exchange for a modest but real score advantage. We did not find a way to close this gap; it is reported as an open, and now honestly small, tradeoff — not a decisive case for either choice.
  - `ConversationBrain`'s `ConversationState` also does not share `SharedSessionState`'s contract in `interfaces.py` — retrieval still reads its own constraint list, populated by `pipeline/router.py`, never `ConversationBrain`'s `known_constraints`. Unifying the two remains undone.
  - We noticed while reading `dialog.py` that `choose_next_attribute` is defined twice; the first definition (which references an undefined `ATTRIBUTE_PRIORITY` name) is dead code, silently shadowed by the second. Harmless at runtime, worth cleaning up.
  - We also noticed `ConversationBrain.observe()` *removes* a constraint from `known_constraints` when an override is detected. Our own `router.py` deliberately keeps both the old and new value, because both describe the same target product and measurably improve override-scenario MRR (0.904, the strongest of the four scenarios) when kept together. This is a genuine, unresolved design disagreement between the two contracts, not a bug — worth reconciling before merging the two state objects.
