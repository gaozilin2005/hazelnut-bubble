...
- **Multi-route lexical RRF** (`--rrf`): four lexical queries fused by reciprocal rank. The one idea whose mechanism matched the saturation diagnosis — still **−0.0004** paired (p=0.0026). The broad route recovers a few out-of-bucket targets (+0.002 Hit), but fusion costs more MRR than that's worth.
- **Broad-pool augmentation** (`--broad-pool`): keep RRF's extra candidates, drop the fusion. **−0.121**, 387/1,000 sessions worse — hundreds of out-of-bucket products score identically to the target once coverage saturates, so the category bucket is the only precision mechanism left, and RRF's rank-agreement was the only thing holding that noise back.
...
- **Dense-vector fusion into ranking**: monotonically harmful at every weight (0.8802 → 0.8464, 0→0.35). A disclosed constraint is a near-verbatim slice of the target's own metadata, so exact matching identifies the item; semantic neighbours are plausible-but-wrong.
...
- **Leave-one-out constraint tolerance**: −0.007. Dropping a session's worst-matching constraint dilutes discrimination more than it rescues.
- **Hard-filtering on disclosed constraints** instead of soft-weighted scoring: filtering the first constraint alone was a no-op; filtering every constraint was worse (−0.004) — override sessions' old/new wording doesn't always both appear verbatim, so requiring both can exclude the true target.
- **Amplifying the first-stated constraint's weight** (1.5×–5×): no effect at any multiplier — candidates in a bucket for a common constraint mostly already share full coverage on it.
- **Attribute-by-attribute clarification**: −0.039 held-out, widening to −0.020 out of sample. `ask_attribute="other"` dominates by construction — see [The Clarification Channel Has a Dominant Strategy](#the-clarification-channel-has-a-dominant-strategy).
- **Intent-override slot erasure** (`--erase-on-override`): −0.006 held-out, override MRR 0.834→0.763. The simulator's old/new values share one intent card and never truly conflict, so erasing discards a valid signal.
- **Widening the reranker's pool** (10→200): no gain at any width, mildly negative past 100 — 29/38 remaining misses sit at rank 11–50, real headroom but needing a signal we don't have.
- **Exposure-schedule sweep** (15 configs on a separate seed): current setting is already optimal; the best tuning-draw candidate (+0.0005) did not replicate on the clean set.
...
- **Dense-similarity tie-breaking within a narrow band** (`--tie-break-dense`): looked like a genuine win on the held-out draw it was diagnosed on (+0.0061), but an independently-sampled 200-session set gave −0.0019 to +0.0014 — noise-level, wrong sign, and public cost was consistent (−0.014 to −0.021). Did not survive a second sample.
